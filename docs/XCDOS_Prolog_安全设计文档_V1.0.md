...
### 3.3 RBAC 权限模型（三维：资源 × 动作 × 数据范围）

| 角色 | 等级 | 资源范围 | 允许动作 | 数据范围 | 关键限制 |
|------|------|---------|---------|---------|---------|
| boss | 5 | 全组织驾驶舱、决策案例只读 | read | 全组织（跨部门、跨团队） | 不可增删改数据；不替代业务审批 |
| admin | 4 | 全系统管理（用户/组织/Agent/配置） | CRUD | 全组织 | 可管理用户/组织/Agent 配置；不可替代业务审批人 |
| manager | 3 | 本组织全量业务对象（目标/决策/任务/反馈） | CRUD | 本组织及子组织 | 不可跨组织操作；可为直属下级反馈评分 |
| employee | 2 | 名下任务、个人反馈 | read + 提交反馈 + 更新任务状态 | 仅本人被分配的任务 | 不可查看他人任务详情/反馈内容 |
| agent | 1-3 | 按 L1-L5 策略表（见 7.1） | 策略表定义 | 策略表定义 | 高风险写操作须人工确认（见 7.1 可配策略表） |

> **P1-02 关闭说明**：原 PRD 允许 boss 创建目标/审批，安全文档将 boss 定义为只读。**最新裁决**：boss=boss 驾驶舱只读 + 可创建目标/审批决策方案（待产品确认，暂按"允许创建目标 + 审批决策方案"实现，驾驶舱数据为只读）。三维矩阵以本文档为准，PRD 角色描述如有冲突以本文档覆盖。
>
> **P1-03 关闭说明**：admin 区分为平台管理员（tenant_id=0，跨租户只读）、租户管理员（tenant_id=tenant，本租户全量管理）、审计员（只读审计日志）三种子角色。admin 不替代业务审批——审批操作仍须具备对应决策审批权限的 manager/boss 角色执行。
...
| 数据 | 方式 | 说明 |
|------|------|------|
| 用户密码 | Argon2id (m=64MB,t=3,p=4,ADR-0003) | 存储 unified Argon2id 编码，禁止 MD5/bcrypt |
...
### 7.1 Agent 权限分级策略表（P1-04 收敛）

本策略表为唯一事实源，TDD/ARD 的"人工确认"和安全文档的"禁止"统一为可配策略表。`unconfigurable` 表示产品层不可修改的限制。

| 等级 | 允许操作 | 默认策略 | 可配置 | 不可配置（UNCONFIGURABLE） |
|------|---------|---------|--------|---------------------------|
| L1 | 读取数据、分析推理、生成文本 | 自动执行 | N/A | —— |
| L2 | 生成评分、建议、质量评估 | 自动执行 | 可改为人工审核 | —— |
| L3 | 创建任务/方案/草稿规则 | 人工确认 | 可改为自动（仅试点租户、<10% 流量） | Agent 不得审批自己创建的资源 |
| L4 | 修改数据、关闭异常、规则发布 | 强制人工确认 | 不可改为自动 | Agent 不得修改权限/角色/密码/租户配置 |
| L5 | 删除数据、修改系统配置、租户隔离切换 | 禁止 Agent 执行（系统层硬限制） | 不可启用 | **绝对禁止**：删除/系统配置/租户切换/API Key 管理。代码层 return 403 before agent_runs 写入 |

**裁决逻辑**：TDD/ARD 的"L5 人工确认"表述适用于 L4 级别；L5 操作（删除、系统配置、隔离切换）因涉及数据安全与租户边界，代码层硬拒绝，不进入 agent_runs 表，不触发人工确认流程。

**策略变更流程**：租户级调整须经安全负责人审批 + 产品负责人确认，变更写入 `tenant_agent_policy` 表并审计留痕。全局默认策略变更须走 ADR。
